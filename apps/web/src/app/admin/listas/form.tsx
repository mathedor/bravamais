...
export function CreateListForm() {
  const [state, action] = useActionState(createListAction, undefined);
  return (
    <form action={action} className="rounded-3xl border border-brava-border bg-brava-card p-5">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block">
          ...
          <input name="slug" required placeholder="top-pizza-sp" className="mt-1 w-full rounded-xl border border-brava-border bg-brava-paper px-3 py-2 text-sm" />
        </label>
        <label className="block">
          <span className="text-xs font-bold uppercase tracking-wider text-brava-muted">Título *</span>
          <input name="title" required placeholder="Top 10 pizzas de SP" className="mt-1 w-full rounded-xl border border-brava-border bg-brava-paper px-3 py-2 text-sm" />
        </label>
        <label className="block sm:col-span-2">
          <span className="text-xs font-bold uppercase tracking-wider text-brava-muted">Descrição</span>
          <input name="description" placeholder="As 10 melhores pra noite de sexta" className="mt-1 w-full rounded-xl border border-brava-border bg-brava-paper px-3 py-2 text-sm" />
        </label>
      </div>
